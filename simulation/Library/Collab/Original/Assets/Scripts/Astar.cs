...
public static class AStar {
  ...
  static List<FastVector3> GetUnobstructedNeighbouringNodes(Vector3 current_point, float grid_granularity = 2f) {
    ...
    foreach (Vector3 neighbour in neighbours) {
      if (IsObstructed(neighbour, current_point)) continue; // do not add obstructed points to returned set
  ...
  static bool IsObstructed(Vector3 point, Vector3 current, float search_boundary_limit = 20f, float sphere_cast_radius = 1f) {
  ...
  public static List<Vector3> FindPath(Vector3 source, Vector3 destination, float near_stop_distance_limit = 3f) {
    ...
    while (frontier_set.Count > 0) {
      ...
      List<FastVector3> neighbours = GetUnobstructedNeighbouringNodes(current_point._vector);
